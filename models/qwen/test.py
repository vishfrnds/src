...
from src.models.qwen.tiny import Qwen
...
class TestQwenModel(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.hub_model_inputs = tokenizer([input_text], return_tensors="pt").to('cpu')

    cls.hub_output = cls.hub_model(cls.hub_model_inputs.input_ids, output_hidden_states=True, output_attentions=True)
    ...
    cls.hub_generate_ids = cls.hub_model.generate(cls.hub_model_inputs.input_ids, max_length=11, eos_token_id=eos_token_ids, do_sample=False)
  ...
  def assert_close(self, tiny: tinygrad.Tensor, hub: torch.Tensor, name: str):
    ...
    for a, b in zip(tiny, hub):
      if abs(a - b) > 1e-4 and abs(a - b) / (2 * (abs(a) + abs(b) + 1e-4)) > 1e-3:
        mismatch += 1
        if mismatch < 10:
    ...
    assert mismatch < 0.001 * len(tiny), f'{mismatch} mismatches'

  def test_model_outputs(self):
    # Get outputs from tiny implementation
    # hub = self.hub_model.model.embed_tokens(self.hub_model_inputs.input_ids)
  ...
  def test_second_call(self):
    ...
    assert tiny_output[-1].item() == self.hub_generate_ids[0][start_pos].item(), f'first_output {tiny_output[0].item()} {self.hub_generate_ids[0][start_pos].item()}'
    for _ in range(10):
      start_pos += 1
